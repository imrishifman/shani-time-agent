...
sudo -u "${DEPLOY_USER}" npm --prefix "${APP_DIR}" install --no-save --package-lock=false --ignore-scripts typescript@5 && sudo -u "${DEPLOY_USER}" env NODE_OPTIONS=--max-old-space-size=1536 "${APP_DIR}/node_modules/.bin/tsc" --noCheck --noResolve -p "${APP_DIR}/tsconfig.json"
